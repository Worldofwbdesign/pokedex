...
const V2_API_URL = 'https://pokeapi.co/api/v2/'

const API = {
  fetchPokemonsList: () =>
    axios.get(`${API_URL}pokedex/1/`).then(res => {
      return res.data.pokemon
    }),

  fetchCurrentPokemon: id =>
    axios.get(`${API_URL}pokemon/${id}/`).then(res => {
      return res.data
    }),

  fetchPokemonTypes: id =>
    axios.get(`${API_URL}type?limit=0`).then(res => {
      return res.data.objects
    }),

  fetchTypeList: type =>
    axios.get(`${V2_API_URL}type/${type}/`).then(res => {
      return res.data.pokemon
    }),
